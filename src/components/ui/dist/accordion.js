...
var AccordionTrigger = React.forwardRef(function (_a, ref) {
    ...
    return (React.createElement(AccordionPrimitive.Header, { className: "flex" },
        React.createElement(AccordionPrimitive.Trigger, __assign({ ref: ref, className: utils_1.cn("flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all hover:underline text-left [&[data-state=open]>svg]:rotate-180", className) }, props),
            children,
            React.createElement(lucide_react_1.ChevronDown, { className: "h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200" }))));
});
